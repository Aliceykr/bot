...
> 🌐 语言 / Language: **简体中文** | [English](README.en.md) | [Français](README.fr.md) | [Español](README.es.md) | [Русский](README.ru.md) | [العربية](README.ar.md)
...
400kHz I2C，±2g 量程，DLPF 44Hz。仅在进入 2048 游戏时初始化，退出后释放总线。进入游戏后会先采样当前姿态作为中立基线，再用滞后阈值（ENTER=0.30g, EXIT=0.15g）+ 方向锁定判断倾斜，X/Y 主导轴决定方向。这样可减少安装偏角、零偏和回弹抖动导致的误触发。
...
- 2048 倾斜控制：MPU6050 加速度计在进入游戏时先校准当前姿态为中立点，再将设备倾斜映射到上下左右四个方向，与按键并存（按键优先，无按键时启用倾斜）
