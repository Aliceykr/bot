...
void LCD_GPIO_Init(void)
{
    ...
    io_conf.pin_bit_mask = (1ULL<<LCD_MOSI_PIN)|(1ULL<<LCD_SCLK_PIN)|
                           (1ULL<<LCD_RES_PIN) |(1ULL<<LCD_DC_PIN)  |
                           (1ULL<<LCD_BLK_PIN);
...
void LCD_Writ_Bus(uint8_t dat)
{
    ...
    for (i = 0; i < 8; i++) {
        LCD_SCLK_Clr();
        if (dat & 0x80) LCD_MOSI_Set(); else LCD_MOSI_Clr();
        LCD_SCLK_Set();
        dat <<= 1;
    }
}
...
void LCD_Init(void)
{
    ...
    LCD_WR_DATA8(0x12);
    LCD_WR_REG(0xC5);
    LCD_WR_DATA8(0x33); LCD_WR_DATA8(0x3F);
    LCD_WR_REG(0xC7);
    LCD_WR_DATA8(0x92);
    LCD_WR_REG(0x3A);
    LCD_WR_DATA8(0x55);
    ...
    LCD_WR_REG(0xB1);
    LCD_WR_DATA8(0x00); LCD_WR_DATA8(0x12);
    LCD_WR_REG(0xB6);
    LCD_WR_DATA8(0x0A); LCD_WR_DATA8(0xA2);
    LCD_WR_REG(0x44);
    LCD_WR_DATA8(0x02);
    LCD_WR_REG(0xF2);
    LCD_WR_DATA8(0x00);
